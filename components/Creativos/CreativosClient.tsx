...
type Action = 'scale' | 'maintain' | 'watch' | 'pause' | 'no_data'
...
function classify(ad: CreativeRow): { action: Action; reason: string } {
  ...
  if (roas && roas >= ROAS_SCALE) return { action: 'scale', reason: `ROAS ${roas.toFixed(2)}x excepcional. Duplicar budget o replicar en más ad sets.` }
  ...
  if (roas && roas < ROAS_MIN && spend > 3000) return { action: 'pause', reason: `ROAS ${roas.toFixed(2)}x por debajo del mínimo (${ROAS_MIN}x). No es rentable.` }
  if (roas && roas >= ROAS_MIN && results > 0) return { action: 'maintain', reason: `Performance estable. ROAS ${roas.toFixed(2)}x dentro de rango.` }
  return { action: 'watch', reason: 'Datos insuficientes para recomendación definitiva.' }
}
...
export default function CreativosClient({ snapshot }: Props) {
  ...
  if (!snapshot) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center">
        ...
        <p className="text-sm text-gray-500 dark:text-zinc-500 max-w-xs">Ejecutá un sync para ver el análisis de creativos.</p>
      </div>
    )
  }
  ...
  const rows: CreativeRow[] = snapshot.ads
    ...
    .map(a => {
      ...
      const { action, reason } = classify(base)
      return { ...base, action, reason }
    })

  const counts = { scale: 0, maintain: 0, watch: 0, pause: 0, no_data: 0 }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-gray-900 dark:text-zinc-100">Análisis de creativos</h1>
        <p className="text-sm text-gray-500 dark:text-zinc-500 mt-0.5">{rows.length} anuncios · datos últimos 7d · {snapshot.snapshot_date}</p>
      </div>

      {/* Filter chips */}
      <div className="flex flex-wrap gap-2">
        {(['all', 'scale', 'pause', 'watch', 'maintain', 'no_data'] as const).map(f => {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-400 dark:text-zinc-500 bg-gray-50 dark:bg-zinc-800/50 border-b border-gray-100 dark:border-zinc-800">
                ...
                <th className="text-left px-4 py-2.5 font-medium">Recomendación</th>
